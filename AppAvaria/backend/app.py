...
app.config['SECRET_KEY'] = "TROQUE_ESSA_CHAVE"
...
def create_user():
    ...
    db.session.add(user)
    db.session.commit()
    return jsonify({'msg':'Usuário criado','user':{'username':user.username,'role':user.role}}),201

# ROTA PROTEGIDA
@app.route('/protected', methods=['GET'])
@token_required
def protected():
    return jsonify({'msg':'Você está autenticado','user':{'username':request.user.username,'role':request.user.role}})
